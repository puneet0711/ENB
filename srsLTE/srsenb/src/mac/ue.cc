...
#include "srsenb/hdr/phy/phch_worker.h"  // Author : Puneet Sharma
...
int NBRVAL=0; // Author : Puneet Sharma
...
int32_t test;
  ...
  switch(subh->ce_type()) {
	...
	if(counter < 1){                                 // Author : Puneet Sharma
        NBRVAL = buff_size[0]-2;                           // Author : Puneet Sharma
        variable(NBRVAL);
	Info("the value if PRBlen = %d \n",NBRVAL );          // Author : Puneet Sharma
	counter++;
